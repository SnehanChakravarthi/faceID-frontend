    ...
    <div className="flex flex-col justify-center w-full mx-auto gap-4">
      <div
        className={cn(
          ...
          (isAuthentication || success) && 'hidden'
        )}
      >
        <div
          className={cn(
      ...
      >
        <div
          className={cn(
            ...
            error && 'shadow-[inset_0_0_100px_6px_rgba(239,68,68,1)]'
          )}
        ></div>
        {/* <div
        ...
        {capturedFrames.length > 0 ? (
          <div className="relative  w-full h-80 ">
            ...
            <button
              onClick={handleRetake}
              className={cn(
                'absolute bottom-4 left-1/2 z-50 -translate-x-1/2 px-6 py-2.5 rounded-full text-sm font-medium bg-white/60 backdrop-blur-sm hover:bg-white/90 transition-all duration-200 shadow-lg text-neutral-900 opacity-0 group-hover:opacity-100',
                success && 'hidden'
              )}
            >
              Retake Photo
          ...
          <div className={cn('relative w-full h-80 group')}>
            ...
            {/* Capture Button */}
            {isCameraReady && !isRecording && !success && (
